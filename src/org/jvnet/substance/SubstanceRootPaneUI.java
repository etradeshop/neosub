 ...
 */
public class SubstanceRootPaneUI extends MetalRootPaneUI {
	/**
	...
	private void installClientDecorations(JRootPane root) {
		...
		installWindowListeners(root, root.getParent(), titlePane);
		installLayout(root);
		if (window != null) {
			root.revalidate();
			root.repaint();
	...
	private class MouseInputHandler implements MouseInputListener {
		 ...
		 */
		// private final PrivilegedExceptionAction<?> getLocationAction = new
		...
		public void mousePressed(MouseEvent ev) {
			...
			Window w = titlePanes.get((JComponent) ev.getSource());
			if (w != null) {
				w.toFront();
			}
			...
			if (isMovingWindow) {
				...
				if (dragCursor != 0) {
					...
					if (!r.equals(startBounds)) {
						w.setBounds(r);
						// Defer repaint/validate on mouseReleased unless
						// dynamic
						// layout is active.
	...
	public void paint(Graphics g, JComponent c) {
		...
		if (parent != null) {
			if (parent instanceof Dialog) {
				...
				if (parentColor.getRGB() == SystemColor.control.getRGB())
					parent.setBackground(UIManager
							.getColor("RootPane.background"));
			}
		}
		super.paint(g, c);
	}
